...
export const getContentHashLink = (decodedContentHash: any) => {
...
export const contentHashToString = (hash: any): string => {
...
export const encodeContentId = (
  ...
  try {
    if (protocolType === "ipfs" && contentId?.length >= 4)
      ...
      ["arweave", "ar"].includes(protocolType) &&
      contentId.length === 43
    )
...
export const encodeContenthash = (address: string) => {
  ...
  return encodeContentId(protocol as ContentHashProtocol, contentId as string);
};
